%%--------------------Chapter 3------------------------
\chapter{How I Made It Work} \labchap{thetis_design}
...
\subsection{Revision F1} \labsubsec{revision_f1}
...
\subsection{Revision F2} \labsubsec{revision_f2}
...
\subsection{Revision F3} \labsubsec{revision_f3}
...
\subsection{Revision F4} \labsubsec{revision_f4}
...
\begin{figure}[h!]
	\centering
	\includegraphics[height=3in]{design/tht_vs_smd.jpg}
	\caption[Comparison of through hole and SMT soldering]{A comparison between through hole (THT) (top) and surface mount (SMT) (bottom).
	Retrieved from \href{https://www.yamanelectronics.com/wp-content/uploads/2022/05/through-hole-vs-smd-components.jpg}{Yaman Electronics}}
	\labfig{tht_vs_smt}
\end{figure}
...
When the PCB is secured, a line of paste is placed on one side and smoothly dragged along the cutouts with even pressure applied throughout.
When the stencil is removed, there should be a healthy deposit of solder paste on each pad as shown in of Figure \ref{subfig:assembly_pasted}

\begin{figure}[h!]
    \centering
    \subfloat[Thetis placed into a jig, ready to put under the stencil.]{\includegraphics[width=0.5\textwidth, angle=90, origin=c]{design/assembly_jig}\label{subfig:assembly_jig}}\hskip3ex
    \subfloat[Depositing the solder on the stencil and dragging it across.]{\includegraphics[width=0.5\textwidth, angle=-90, origin=c]{design/assembly_paste.jpg}\label{subfig:assembly_paste}}\hskip3ex
    \subfloat[Solder deposited on the PCB pads from the stencil]{\includegraphics[width=0.5\textwidth, angle=-90, origin=c]{design/assembly_pasted.jpg}\label{subfig:assembly_pasted}}
	\caption[Solder deposition for stencil PCB assembly]{A series showing the process of putting solder paste onto a PCB from a stencil using Revision F5.}
	\labfig{stencil_process}
\end{figure}
...
\paragraph*{Reflow Soldering} When all of the components are placed, the PCB can be placed onto a hotplate for reflow.
The hotplate heats the PCB and solder up to the melting point (approximately 250\textdegree C) where the flux in the paste evaporates and the metallic solder is left behind.
Solder paste typically has a reflow profile provided by the manufacturer to ensure optimal mechanical strength and solder characteristics, but this is ignore for simplicity.
For a more precise reflow, an infrared or convection oven can be used where the optimal reflow profile is programmed into the machine and the board and components are more evenly and thoroughly heated.
...
\begin{figure}[h!]
    \centering
    \subfloat[Components being placed from inventory boxes onto the boards manually using tweezers]{\includegraphics[width=0.5\textwidth, angle=-90, origin=c]{design/assembly_placing_components}\label{subfig:assembly_placing}}\hskip3ex
    \subfloat[All in-stock components placed on the PCB.]{\includegraphics[width=0.5\textwidth, angle=-90, origin=c]{design/assembly_placed_components.jpg}\label{subfig:assembly_placed}}\hskip3ex
    \subfloat[PCB on a mini hotplate for solder reflow.]{\includegraphics[width=0.5\textwidth, angle=-90, origin=c]{design/assembly_reflow.jpg}\label{subfig:assembly_reflow}}
	\caption[Component placement and reflow for a Thetis Revision F5 PCB]{A series showing the process of placing components onto a Revision F5 PCB from an inventory and using a hotplate for solder reflow.}
	\labfig{assembly_process}
\end{figure}
...
If a soldering defect is discovered, the board can be placed onto a small heating plate to melt the solder.
...
Once the pads are cleaned, either hot air or a hot plate can be used to heat the afflicted area and remelt the solder so the part can be replaced on the board.
The board is inspected again and if the issue is cleared, the board is ready for testing.

\begin{figure}[h!]
	\centering
	\includegraphics[height=2in, angle=-90, origin=c]{design/assembly_inspection.jpg}
...
\begin{figure}[h!]
	\centering
	\includegraphics[height=2in]{design/assembly_testing.jpg}
...
\section{Prototype Problems} \labsec{prototype_problems}
